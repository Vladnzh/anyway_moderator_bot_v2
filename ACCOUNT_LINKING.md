...
Система отправляет POST запрос на админку в следующих случаях:
- При автоматической реакции (status: "approved")
- **При добавлении в модерацию** (status: "pending")
...
- **При прямой постановке реакции через API** `/api/reactions/set` (status: "approved")
...
#### Поиск данных для API реакций

При прямой постановке реакции через API `/api/reactions/set` система:

1. **Ищет данные в очереди модерации** по `chat_id` и `message_id`
2. **Если не найдено, ищет в логах** по `chat_id` и `message_id`  
3. **Если данные найдены** - отправляет полную информацию о пользователе и теге
4. **Если данные не найдены** - отправляет минимальные данные (только `chat_id`, `message_id`, `emoji`)
...
Все запросы содержат поле `status` со значениями:
- **"approved"** - реакция поставлена (автоматически или после одобрения модератором)
- **"pending"** - сообщение добавлено в модерацию, ожидает решения
