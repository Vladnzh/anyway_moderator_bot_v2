...
async def handle_any(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if matched_tag['moderation_enabled']:
        ...
        # Отправляем данные при добавлении в модерацию
        logger.debug("📊 Отправляем данные о сообщении в модерации на бэкенд...")
        await send_reaction_data(message, matched_tag, media_info, thread_name, "pending")
